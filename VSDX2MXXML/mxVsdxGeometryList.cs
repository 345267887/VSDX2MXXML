...
{
    class mxVsdxGeometryList
    {
        private IList<mxVsdxGeometry> geomList = new List<mxVsdxGeometry>();
        private IList<mxVsdxGeometry> parentGeomList = null;
        ...
        public virtual void addGeometry(XmlElement geoElem)
        {
                ...
                if (!geo.NoShow)
                {
                    ...
                    foreach (Row row in rows)
                    {
                        ...
                        else if (row is LineTo)
                        {
                            ...
                            double x = row.X != null ? row.X : 0, y = row.Y != null ? row.Y : 0;
