...
//ORIGINAL LINE: @SuppressWarnings("boxing") @Override public String toString()
			...
			{ // OK to ignore boxing here
              //return string.Format("{0}[buffer={1}, currentLinePos={2}, eof={3}, ibitWorkArea={4}, lbitWorkArea={5}, " + "modulus={6}, pos={7}, readPos={8}]", this.GetType().Name, Arrays.ToString(buffer), currentLinePos, eof, ibitWorkArea, lbitWorkArea, modulus, pos, readPos);
                return string.Format("{0}[buffer={1}, currentLinePos={2}, eof={3}, ibitWorkArea={4}, lbitWorkArea={5}, " + "modulus={6}, pos={7}, readPos={8}]", this.GetType().Name, (buffer).ToString(), currentLinePos, eof, ibitWorkArea, lbitWorkArea, modulus, pos, readPos);
            }
		}
		...
		protected internal readonly sbyte PAD = PAD_DEFAULT; // instance variable just in case it needs to vary later
...
//ORIGINAL LINE: protected byte[] ensureBufferSize(final int size, final Context context)
		protected internal virtual sbyte[] ensureBufferSize(int size, Context context)
		{
...
//ORIGINAL LINE: protected static boolean isWhiteSpace(final byte byteToCheck)
		...
		{
            char temp= Convert.ToChar(byteToCheck);
			switch (temp)
			{
				case ' ' :
				case '\n' :
