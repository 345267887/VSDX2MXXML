...
namespace VSDX2MXXML
{
    class mxVsdxGeometry
    {
        ...
        public mxVsdxGeometry(XmlElement elem)
        {
            index = int.Parse(elem.GetAttribute("IX"));
            processGeoElem(elem);
        }

        public mxVsdxGeometry(XmlElement elem, IList<mxVsdxGeometry> parentGeo)
        {
            index = int.Parse(elem.GetAttribute("IX"));
        ...
        private void processGeoElem(XmlElement elem)
        {
            List<XmlElement> cellElems = mxVsdxUtils.getDirectChildNamedElements(elem, "Cell");
            List<XmlElement> rowElems = mxVsdxUtils.getDirectChildNamedElements(elem, "Row");
            ...
            foreach (XmlElement cellElem in cellElems)
            {
            ...
            foreach (XmlElement rowElem in rowElems)
            {
